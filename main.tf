...
/* ------- AWS Lambda ------ */
 /*data "aws_iam_role" "group1-tf-cp2-lambda-role" {
   name = "group1-capstone2-gettodos-lambda-role"
 }

 data "archive_file" "group1-lambda-zip" {
   type        = "zip"
   source_file = "./resources/lambda.py"
   output_path = "./resources/lambda_function.zip"
 }

 resource "aws_lambda_function" "group1-tf-cp2-lambda-create" {
   function_name    = "group1-tf-cp2-lambda"
   filename         = data.archive_file.group1-lambda-zip.output_path
   source_code_hash = data.archive_file.group1-lambda-zip.output_base64sha256
   role             = data.aws_iam_role.group1-tf-cp2-lambda-role.arn
   handler          = "group1-tf-cp2-lambda.lambda_handler"
   runtime          = "python3.9"
 }*/
...
resource "aws_codebuild_project" "group1-tf-cp2-cb-create" {
  name         = "group1-tf-cp2-codebuild"
  description  = "group1-tf-codebuild-project"
  ...
  environment {
    ...
    environment_variable {
      ...
      value = aws_ecr_repository.group1-tf-cp2-ecr-repo.id
    }
    environment_variable {
      name  = "IMAGE_TAG"
      value = "latest"
    ...
    environment_variable {
      name  = "CLUSTER_NAME"
      value = aws_ecs_cluster.group1-tf-cp2-cluster.id
    }
    environment_variable {
      name  = "SERVICE_NAME"
      value = aws_ecs_service.group1-tf-cp2_service.id
    }
  }

  source {
    type            = "GITHUB"
    location        = "https://github.com/belannabarnes/group1capstone.git"
    git_clone_depth = 1
  }
} 
...
/* ------- CodePipeline ------ */
resource "aws_codepipeline" "group1-tf-cp2-cp-create" {
  name     = "group1-tf-cp2-codepipeline"
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        "ConnectionArn"    = aws_codestarconnections_connection.group1-tf-cp2-connection.arn
        "FullRepositoryId" = "belannabarnes/group1capstone"
        "BranchName"       = "master"
      }
    }
  }
  ...
  stage {
    ...
    action {
      ...
      configuration = {
        "ProjectName" = aws_codebuild_project.group1-tf-cp2-cb-create.id
      }
    }
  }
}
